...
{
    private static readonly byte[] Magic = Encoding.ASCII.GetBytes("R2KKEY01");
    ...
    public async Task ExportAsync(ManagedKey managedKey, string destinationPath, string password, CancellationToken cancellationToken = default)
    {
        ...
        if (File.Exists(destinationPath))
            throw new IOException("The selected .r2kkey file already exists. Rice2k will not overwrite it automatically.");

        var directory = Path.GetDirectoryName(Path.GetFullPath(destinationPath));
        if (string.IsNullOrWhiteSpace(directory) || !Directory.Exists(directory))
            throw new DirectoryNotFoundException("The selected key-package folder does not exist.");
        ...
        try
        {
            ...
            cancellationToken.ThrowIfCancellationRequested();
            if (File.Exists(destinationPath))
                throw new IOException("A file appeared at the selected destination before Rice2k could finalize the key package.");
    ...
    public async Task<ManagedKey> ImportAsync(string sourcePath, string password, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(sourcePath);
        ValidatePassword(password, creatingPackage: false);

        if (!File.Exists(sourcePath))
            throw new FileNotFoundException("The selected .r2kkey file could not be found.", sourcePath);
        ...
        try
        {
            ...
            if (!magic.SequenceEqual(Magic))
                throw new InvalidDataException("This is not a supported Rice2k key package.");

            var version = reader.ReadByte();
            var kdf = reader.ReadByte();
            if (version != Version)
                throw new NotSupportedException($"Rice2k key-package version {version} is not supported by this build.");
            if (kdf != KdfArgon2Id)
                throw new NotSupportedException("This key package uses an unsupported password derivation method.");

            var opsLimit = reader.ReadInt64();
            var memLimit = reader.ReadInt32();
            if (opsLimit < 3 || opsLimit > MaximumSupportedOpsLimit)
                throw new InvalidDataException("The key package contains an unsupported Argon2id work factor.");
            if (memLimit < 8 * 1024 * 1024 || memLimit > MaximumSupportedMemLimit)
                throw new InvalidDataException("The key package contains an unsupported Argon2id memory setting.");
            ...
            if (cipher.Length != cipherLength || input.Position != input.Length)
                throw new InvalidDataException("The key package is truncated or contains unexpected trailing data.");
            ...
            {
                throw new CryptographicException("Rice2k could not unlock this key package. The password may be wrong, or the package may have been modified.", ex);
            }
            ...
            {
                throw new InvalidDataException("The decrypted key package contains invalid key data.", ex);
            }
        ...
        {
            throw new InvalidDataException("The key package appears to be truncated.", ex);
        }
        finally
        {
            CryptographicOperations.ZeroMemory(passwordBytes);
    ...
    private static void ValidatePassword(string password, bool creatingPackage)
    {
        ...
        if (creatingPackage && password.Length < MinimumPasswordLength)
            throw new ArgumentException($"Use a password of at least {MinimumPasswordLength} characters for exported key packages.");
    }
    ...
    private static void TryDelete(string path)
    {
        ...
        {
            // Best-effort temporary-file cleanup. The original key remains in memory.
        }
    }
}
