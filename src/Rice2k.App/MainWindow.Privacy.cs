...
public partial class MainWindow
{
    ...
    private bool ClearTransientSensitivePreviews()
    {
        ...
        // Password-only file crypto evaluates the PasswordBox value when the service
        // call starts, so clearing the visible fields does not change the running job.
        // The password+key-file workflow, however, still has an await between package
        // unlock and its later file-crypto call. Preserve those masked controls while
        // that operation is active so App Lock/Privacy Mode cannot turn a privacy scrub
        // into an operation failure. Its finally block clears all of them afterward.
